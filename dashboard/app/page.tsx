...
export default async function Page({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 md:py-10">
      ...
      <div className="flex flex-wrap gap-1.5 mb-6 glass-panel rounded-2xl p-1.5 w-fit shadow-sm">
        {FILTERS.map((f) => (
          <Link
            key={f.value}
      ...
      {error ? (
        <div className="rounded-xl bg-red-50/80 border border-red-100 p-4 text-red-700">
          שגיאה בטעינת פניות
        </div>
      ) : !tickets || tickets.length === 0 ? (
        <div className="text-center py-20 text-gray-400 glass-panel rounded-2xl border-dashed">
        ...
        <div className="space-y-2.5">
          {tickets.map((ticket: Ticket) => (
            <Link
              ...
              className="group flex items-center justify-between glass-panel rounded-2xl px-5 py-4 hover:border-indigo-200/60 hover:shadow-xl hover:shadow-purple-500/10 hover:-translate-y-0.5 transition-all"
            >
              <div className="flex items-center gap-3.5">
                <div className="relative w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-sm shadow-indigo-500/25">
